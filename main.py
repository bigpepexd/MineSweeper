    ...
    def left_clicked(self):
        ...
        if pygame.sprite.groupcollide(cursor_group, flag_group, dokilla=False, dokillb=False):
            ...
            for block in block_group:
                if pygame.sprite.spritecollideany(block, cursor_group):
                    pygame.sprite.groupcollide(cursor_group, block_group, dokilla=False, dokillb=True)
                    if pygame.sprite.spritecollideany(block, bomb_group):
                        pygame.sprite.groupcollide(block_group, bomb_group, dokilla=True, dokillb=False)
                    elif not pygame.sprite.groupcollide(nums_group, cursor_group, dokilla=False, dokillb=False):
                        for block in block_group:
                            for cursor in cursor_group:
                                if not pygame.sprite.spritecollideany(block, flag_group):
...
mine_position = 0
num_of_bombs = 49
random_num = random.sample(range(257), k=num_of_bombs)
